...
def remove_ci_from_reestr():
    try:
        ...
        for i in delete:
            row = [str(i) for i in [r for r in session.execute(
                f"""SELECT * from test.app where id={ci.iloc[i].id}""")]]
...
def see_N_last_added():
    try:
        ...
        ci = pd.DataFrame(session.execute(
            f"""SELECT * FROM test.app LIMIT {n_row}"""))
        ci = ci[ci['delete_time'].isnull()]
